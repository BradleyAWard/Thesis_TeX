...
\begin{equation}
    \frac{dN}{dV(z)} \sim \sum_i^N \frac{1}{V_{\textrm{max,i}}(z)},
\label{eq:number_density_vmax}
\end{equation}

\noindent where $V_{\textrm{max,i}}$ is the maximum comoving volume (up to the maximum redshift of the bin) in which the $i$th galaxy in this bin could be detected in the survey.

[Volume calculation - $V_{\textrm{max}} = \int \frac{dV}{dz} \Omega dz$]

[$\phi(M,z) = \frac{1}{\Delta M}\frac{dN}{dV} = \frac{1}{\Delta M}\sum\frac{1}{V}$, with dependencies]
